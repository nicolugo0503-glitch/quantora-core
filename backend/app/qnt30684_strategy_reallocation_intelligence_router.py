...
router = APIRouter(tags=["strategy-reallocation-intelligence"])

PROJECT_DIR = Path(__file__).resolve().parents[2]
ARTIFACTS_DIR = PROJECT_DIR / "backend" / "artifacts"
REALLOC_DIR = ARTIFACTS_DIR / "strategy_reallocation_intelligence"

DEFAULT_POLICY = {
    "minimum_reallocation_score": 84.0,
    "minimum_rotation_score": 80.0,
    "minimum_defense_score": 78.0,
    "minimum_restoration_score": 78.0,
    "minimum_recovery_score": 76.0,
...
def _path(email: str) -> Path:
    REALLOC_DIR.mkdir(parents=True, exist_ok=True)
    return REALLOC_DIR / f"{_safe(email)}.json"
...
def _rows(inputs: dict, policy: dict) -> list[dict]:
    ...
    target_quality_score = max(0.0, min(100.0,
        rotation_score * 0.26 +
        defense_score * 0.18 +
        restoration_score * 0.16 +
        continuity_score * 0.12 +
        governance_score * 0.12 +
        compliance_score * 0.10 +
        max(0.0, 100.0 - min(drawdown_pct * 2.0, 28.0)) * 0.06
    ))

    exception_pressure = 0.0
    exception_pressure += float(rotation_run.get("escalate_count") or 0.0) * 5.0
    exception_pressure += float(defense_run.get("escalate_count") or 0.0) * 4.0
    ...
    reallocation_raw = (
        ...
        max(0.0, 100.0 - min(concentration_pct * 0.8, 24.0)) * 0.04
    )
    reallocation_score = max(0.0, min(100.0, reallocation_raw - exception_pressure - break_pressure))

    return [{
        "portfolio_id": "STRATEGY_REALLOCATION_BOOK",
...
def _decisions(rows: list[dict], policy: dict) -> list[dict]:
    decisions = []
    for row in rows:
        score = float(row.get("reallocation_score") or 0.0)
        ...
        if score < policy["minimum_reallocation_score"]:
            action = "REBALANCE"
            reasons.append("reallocation score below threshold")
        if concentration_pct > 48.0 and action in {"REALLOCATE", "REBALANCE"}:
            action = "REDUCE"
            reasons.append("concentration too high for safe reallocation")
        ...
        if drawdown_pct > policy["maximum_drawdown_pct"] * 1.2:
            action = "ESCALATE"
            reasons.append("drawdown exceeds reallocation tolerance")
        elif target_quality_score < policy["minimum_target_quality_score"] and action == "REALLOCATE":
            action = "REBALANCE"
            reasons.append("target quality below threshold")
        if not reasons:
            reasons.append("reallocation posture inside governed band")
        ...
        decisions.append({
            ...
            "reallocation_score": row.get("reallocation_score"),
        })
    return decisions
...
def _overview(rows: list[dict], decisions: list[dict]) -> dict:
    if not rows:
        return {
            ...
            "hold_count": 0,
            "escalate_count": 0,
            "reallocatable_capital_millions": 0.0,
        }
    score = sum(float(r.get("reallocation_score") or 0.0) for r in rows) / len(rows)
    cap = sum(float(r.get("governed_capital_millions") or 0.0) for r in rows)
    counts = {"REALLOCATE": 0, "REBALANCE": 0, "REDUCE": 0, "HOLD": 0, "ESCALATE": 0}
    for d in decisions:
        counts[d.get("action")] = counts.get(d.get("action"), 0) + 1
    posture = "ACTIVE_REALLOCATION"
    if counts["ESCALATE"] > 0:
        ...
        posture = "REBALANCING"
    return {
        "reallocation_posture": posture,
        "reallocation_score": _round_pct(score),
        "reallocate_count": counts["REALLOCATE"],
        "rebalance_count": counts["REBALANCE"],
        "reduce_count": counts["REDUCE"],
        "hold_count": counts["HOLD"],
        "escalate_count": counts["ESCALATE"],
        "reallocatable_capital_millions": _round_money(cap),
    }
...
def _build_summary(email: str):
    ...
    return {
        ...
        "policy": policy,
        "strategy_reallocation_overview": overview,
        "reallocation_book": book,
        "reallocation_decisions": decisions,
        "reallocation_dependencies": {
        ...
        "reallocation_agenda": _agenda(decisions),
    }

@router.get("/api/strategy-reallocation-intelligence/summary")
def strategy_reallocation_intelligence_summary():
    session = _require_user()
    return _build_summary(session.get("email"))

@router.post("/api/strategy-reallocation-intelligence/run")
def strategy_reallocation_intelligence_run(payload: dict = Body(default=None)):
    session = _require_user()
    email = session.get("email")
    store = _load(email)
    summary = _build_summary(email)
    overview = summary.get("strategy_reallocation_overview") or {}
    run = {
        "run_id": f"sri_{time.time_ns()}",
        "mission": "QNT30684",
        "trigger": (payload or {}).get("trigger") or "manual",
        "timestamp": _now_ts(),
        "generated_at": summary.get("generated_at"),
        "reallocation_posture": overview.get("reallocation_posture"),
        "reallocation_score": overview.get("reallocation_score"),
        "reallocate_count": overview.get("reallocate_count"),
        ...
        "reallocatable_capital_millions": overview.get("reallocatable_capital_millions"),
    }
...
@router.get("/api/strategy-reallocation-intelligence/audit")
def strategy_reallocation_intelligence_audit():
    session = _require_user()
    email = session.get("email")
    store = _load(email)
    runs = store.get("runs") or []
...
@router.post("/api/strategy-reallocation-intelligence/policy")
def strategy_reallocation_intelligence_policy(payload: dict = Body(...)):
    session = _require_user()
    email = session.get("email")
    store = _load(email)
    policy = store.get("policy") or dict(DEFAULT_POLICY)
