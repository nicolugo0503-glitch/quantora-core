...
def build_allocator_report(operator_id: str):
    return {
...
def ensure_user_state(user):
    ...
    save_json("passport.json", build_passport(operator_id))
    save_json("passport_score.json", build_score())
    save_json("violation_registry.json", build_violations(operator_id))
...
def auth_register(payload: RegisterRequest):
    ...
    session = {"email": payload.email, "operator_id": operator_id, "display_name": payload.display_name, "logged_in": True}
...
def auth_login(payload: LoginRequest):
    ...
    session = {"email": user["email"], "operator_id": user["operator_id"], "display_name": user["display_name"], "logged_in": True}
    save_json("session.json", session)
    ensure_user_state(user)
