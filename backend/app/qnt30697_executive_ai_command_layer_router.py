...
EXEC_DIR = ARTIFACTS_DIR / "executive_ai_command_layer"

DEFAULT_POLICY = {
    "minimum_executive_score": 90.0,
    "minimum_growth_score": 84.0,
    "minimum_autonomy_score": 84.0,
    "minimum_reporting_score": 82.0,
    "minimum_transparency_score": 82.0,
    "minimum_audit_score": 82.0,
    "minimum_governance_score": 84.0,
    "minimum_compliance_score": 84.0,
    "maximum_exception_pressure": 16.0,
    "maximum_break_pressure": 14.0,
    "maximum_drawdown_pct": 10.0,
    "minimum_command_readiness_pct": 84.0,
}
...
def _artifact_inputs(email: str) -> dict:
    return {
        "growth": _read_json(_artifact_file("self_growing_capital_engine", email), {"policy": {}, "runs": []}),
        ...
        "operating_intelligence": _read_json(_artifact_file("operating_intelligence", email), {"policy": {}, "runs": []}),
...
def _rows(inputs: dict, policy: dict) -> list[dict]:
    growth_run = _latest_run(inputs["growth"])
    ...
    operating_run = _latest_run(inputs["operating_intelligence"])
    ...
    growth_score = float(growth_run.get("growth_score") or 0.0)
    ...
    operating_score = float(operating_run.get("operating_score") or operating_run.get("executive_score") or 0.0)

    command_signals = 0
    if growth_run: command_signals += 1
    if autonomy_run: command_signals += 1
    if reporting_run: command_signals += 1
    if audit_run: command_signals += 1
    if governance_run: command_signals += 1
    if ledger_entries: command_signals += 1
    command_readiness_pct = (command_signals / 6.0) * 100.0

    executive_visibility_pct = min(100.0, (((1 if trades else 0) + (1 if positions else 0) + (1 if pnl_ledger else 0) + (1 if allocations else 0) + (1 if operating_run else 0)) / 5.0) * 100.0)

    exception_pressure = min(
        float(growth_run.get("escalate_count") or 0.0) * 5.0 +
        float(autonomy_run.get("escalate_count") or 0.0) * 4.0 +
    ...
    if drawdown_pct > policy["maximum_drawdown_pct"]:
        break_pressure += min(6.0, (drawdown_pct - policy["maximum_drawdown_pct"]) * 0.45)
    if growth_score < policy["minimum_growth_score"]:
        break_pressure += min(6.0, (policy["minimum_growth_score"] - growth_score) * 0.30)
    ...
    if command_readiness_pct < policy["minimum_command_readiness_pct"]:
        break_pressure += min(6.0, (policy["minimum_command_readiness_pct"] - command_readiness_pct) * 0.12)
    ...
    executive_raw = (
        ...
        treasury_score * 0.03 +
        mobility_score * 0.03 +
        operating_score * 0.01 +
        command_readiness_pct * 0.01 +
        executive_visibility_pct * 0.01
    )
    executive_score = max(0.0, min(100.0, executive_raw - exception_pressure - break_pressure))

    return [{
        "executive_scope": "EXECUTIVE_AI_COMMAND",
        ...
        "operating_score": _round_pct(operating_score),
        "command_readiness_pct": _round_pct(command_readiness_pct),
        "executive_visibility_pct": _round_pct(executive_visibility_pct),
        ...
        "executive_score": _round_pct(executive_score),
    }]

def _decisions(rows: list[dict], policy: dict) -> list[dict]:
    decisions = []
    for row in rows:
        score = float(row.get("executive_score") or 0.0)
        readiness = float(row.get("command_readiness_pct") or 0.0)
        ...
        if score < policy["minimum_executive_score"]:
            action = "REVIEW"
            reasons.append("executive score below threshold")
        if readiness < policy["minimum_command_readiness_pct"] and action in {"COMMAND", "REVIEW"}:
            action = "RUNBOOK"
            reasons.append("executive command readiness below threshold")
...
def _overview(rows: list[dict], decisions: list[dict]) -> dict:
    ...
    score = sum(float(r.get("executive_score") or 0.0) for r in rows) / len(rows)
    cap = sum(float(r.get("governed_capital_millions") or 0.0) for r in rows)
    counts = {"COMMAND": 0, "REVIEW": 0, "RUNBOOK": 0, "HOLD": 0, "ESCALATE": 0}
    for d in decisions:
    ...
    return {
        ...
        "executive_score": _round_pct(score),
...
def _agenda(decisions: list[dict]) -> list[dict]:
    return [{
        "sequence": idx,
        "executive_scope": d.get("executive_scope"),
        "action": d.get("action"),
        "reason": d.get("reason"),
...
def _build_summary(email: str):
    ...
    return {
        ...
        "policy": policy,
        "executive_command_overview": overview,
        "executive_command_book": book,
        "executive_command_decisions": decisions,
        "executive_command_dependencies": {
            "growth_latest_run": _latest_run(inputs["growth"]),
            ...
            "operating_intelligence_latest_run": _latest_run(inputs["operating_intelligence"]),
        },
        "executive_command_agenda": _agenda(decisions),
    }

@router.get("/api/executive-ai-command-layer/summary")
def executive_ai_command_layer_summary():
    session = _require_user()
    return _build_summary(session.get("email"))

@router.post("/api/executive-ai-command-layer/run")
def executive_ai_command_layer_run(payload: dict = Body(default=None)):
    session = _require_user()
    email = session.get("email")
    store = _load(email)
    summary = _build_summary(email)
    overview = summary.get("executive_command_overview") or {}
    run = {
        "run_id": f"ecl_{time.time_ns()}",
        "mission": "QNT30697",
        "trigger": (payload or {}).get("trigger") or "manual",
        "timestamp": _now_ts(),
        "generated_at": summary.get("generated_at"),
        "executive_posture": overview.get("executive_posture"),
        "executive_score": overview.get("executive_score"),
        "command_count": overview.get("command_count"),
...
@router.get("/api/executive-ai-command-layer/audit")
def executive_ai_command_layer_audit():
    session = _require_user()
    email = session.get("email")
    store = _load(email)
    runs = store.get("runs") or []
...
def executive_ai_command_layer_policy(payload: dict = Body(...)):
    session = _require_user()
    email = session.get("email")
    store = _load(email)
    policy = store.get("policy") or dict(DEFAULT_POLICY)
