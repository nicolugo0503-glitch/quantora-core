...
router = APIRouter(tags=["institutional-exception-resolution-layer"])
PROJECT_DIR = Path(__file__).resolve().parents[2]
ARTIFACTS_DIR = PROJECT_DIR / "backend" / "artifacts"
ENGINE_DIR = ARTIFACTS_DIR / "institutional_exception_resolution_layer"
DEFAULT_POLICY = {
    "retain_cycles": 180,
    "minimum_resolution_score": 90.0,
    "require_operator_clear": True,
    "require_release_clear": True,
    "require_safety_clear": True,
    "require_recovery_clear": True,
    "require_breach_case": True,
    "require_verified_root_cause": True,
    "max_open_material_exceptions": 1,
...
def _summary_for_email(email: str) -> dict:
    s = _load(email)
    latest = s.get("latest_resolution_run") or {}
    return {
        "institutional_exception_resolution_layer_status": {
            "posture": latest.get("posture", "UNINITIALIZED"),
            "latest_score": latest.get("resolution_score"),
            "resolution_band": latest.get("resolution_band", "UNSET"),
            "resolution_run_count": len(s.get("resolution_runs") or []),
            "alert_count": len(s.get("alerts") or []),
            "operator_review_required": bool(latest.get("operator_review_required", False)),
        },
        "latest_resolution_run": latest,
...
def _score_resolution(payload: dict, ctx: dict, policy: dict) -> dict:
    material_open = int(payload.get("open_material_exceptions") or 0)
    critical_open = int(payload.get("open_critical_exceptions") or 0)
    exception_type = str(payload.get("exception_type") or "CONTROL").upper()
    ...
    requires_committee_revote = bool(payload.get("requires_committee_revote", False))
    ...
    if critical_open > int(policy.get("max_open_critical_exceptions", 0)):
        blockers.append("OPEN_CRITICAL_EXCEPTIONS_PRESENT")
        score -= 22
    if material_open > int(policy.get("max_open_material_exceptions", 1)):
        flags.append("OPEN_MATERIAL_EXCEPTIONS_EXCEED_LIMIT")
        score -= 10
    if not root_cause_verified and bool(policy.get("require_verified_root_cause", True)):
        blockers.append("ROOT_CAUSE_NOT_VERIFIED")
        score -= 16
    ...
    if not evidence_complete:
        flags.append("EVIDENCE_NOT_COMPLETE")
        score -= 8
    if requires_committee_revote:
        flags.append("COMMITTEE_REVOTE_REQUIRED")
        score -= 7
    if exception_type in {"MANDATE", "REGULATORY", "INVESTOR", "VALUATION"}:
        flags.append("HIGH_SENSITIVITY_EXCEPTION")
        score -= 6

    if bool(policy.get("require_breach_case", True)) and str(ctx["breach"].get("posture", "")).upper() not in {"CONTAINED", "ESCALATE", "CLEAR"}:
    ...
    operator_review_required = capital_impact_notional >= float(policy.get("operator_review_notional_threshold", 500000.0)) or requires_committee_revote
    if operator_review_required:
        flags.append("OPERATOR_REVIEW_REQUIRED")
        score -= 4

    score = max(score, 0.0)
    minimum = float(policy.get("minimum_resolution_score", 90.0))
    if blockers:
        posture = "BLOCKED"
        band = "UNRESOLVED"
    elif score >= minimum and not operator_review_required:
        posture = "RESOLVED"
        band = "CLEARED"
    ...
    return {
        ...
        "open_material_exceptions": material_open,
        "open_critical_exceptions": critical_open,
    }


@router.get("/api/institutional-exception-resolution-layer/summary")
def summary(session=Depends(_require_user)):
    return _summary_for_email(session.get("email") or "demo@quantora.ai")


@router.post("/api/institutional-exception-resolution-layer/policy")
...
@router.post("/api/institutional-exception-resolution-layer/evaluate")
...
def bootstrap_demo(session=Depends(_require_user)):
    payload = {
        ...
        "requires_committee_revote": False,
    }
