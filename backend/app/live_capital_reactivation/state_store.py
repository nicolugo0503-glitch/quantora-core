...
STATE_PATH = Path(__file__).resolve().parents[1] / 'state' / 'live_capital_reactivation_state.json'


def default_state() -> Dict[str, Any]:
    return {
        'generated_by': 'QNT50029',
        'status': 'degraded',
        'policy': {
            'enabled': True,
            'auto_sync_sources': True,
            'require_reauthorization_execution': True,
            'require_risk_clearance': True,
            'require_strategy_profile_match': True,
            'require_treasury_capacity': True,
            'allow_live_mode': False,
            'max_reactivation_cases': 250,
            'max_reentry_events': 500,
...
def append_audit(event_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    state.setdefault('audit_log', []).insert(0, {
        'event_id': f'live_capital_reactivation_audit_{time.time_ns()}',
