...
class ForensicAuditSystemSmokeTest(unittest.TestCase):
    def setUp(self):
        self.tempdir = tempfile.mkdtemp(prefix="qnt30706_")
        self.email = "operator@example.com"
        names = ["forensic", "audit", "release", "console", "safety", "delivery", "fund_admin", "ledger", "waterfall", "statements", "perf", "ops", "pack", "log", "cal"]
        self.dirs = {}
        for name in names:
    ...
    def test_forensic_lifecycle(self):
        # seed dependent layers
        fund_admin.fund_admin_control_center_bootstrap_demo({"email": self.email})
        delivery.investor_delivery_pack_system_bootstrap_demo({"email": self.email})
        release.institutional_release_control_bootstrap_demo({"email": self.email})
        console.operator_command_console_bootstrap_demo({"email": self.email})
        safety.live_broker_safety_layer_bootstrap_demo({"email": self.email})

        boot = forensic.forensic_audit_system_bootstrap_demo({"email": self.email, "notes": "baseline"})
        self.assertEqual(boot["status"], "bootstrapped")
        self.assertEqual(boot["summary"]["mission"], "QNT30706")
        self.assertGreaterEqual(boot["summary"]["snapshot_count"], 1)
        self.assertGreaterEqual(boot["summary"]["incident_count"], 1)

        snap = forensic.forensic_audit_system_capture_snapshot({"email": self.email})
        self.assertEqual(snap["status"], "captured")

        incident = forensic.forensic_audit_system_reconstruct_incident({"email": self.email, "title": "trade replay anomaly"})
        self.assertEqual(incident["status"], "reconstructed")
        incident_id = incident["incident"]["incident_id"]

        ack = forensic.forensic_audit_system_acknowledge({"email": self.email, "incident_id": incident_id, "status": "closed", "note": "resolved"})
        self.assertEqual(ack["status"], "acknowledged")
        self.assertEqual(ack["acknowledgement"]["incident_id"], incident_id)
